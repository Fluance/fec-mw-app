package net.fluance.cockpit.core.test.mocks.resultset;
...
public class ResultSetMock implements ResultSet {
	...
	private static final DateFormat DATE_ISO_8601 		= new SimpleDateFormat("yyyy-MM-dd");
	private static final String 	NOT_SAME_SIZE 		= "not the same size";

	private List<String[]> rowset;
	private int cursor = 0;
	private Map<String, Integer> columnMap;

	public ResultSetMock(Map<String, Integer> columnMap, String[] row) {
		if (columnMap.size() != row.length) {
			throw new UnsupportedOperationException(NOT_SAME_SIZE);
		}
	...
	public Date parseDateFromString(String value) throws ParseException {
		Date date;
	...
	private String getValue(int columnIndex) throws SQLException {
		...
		if ((columnIndex < 0) || (columnIndex > row.length)) {
			throw new PSQLException("Column Index is null " + columnIndex, PSQLState.UNDEFINED_COLUMN);
		}
	...
	private Integer getColumnIndex(String columnLabel) throws PSQLException {
		Integer index = columnMap.get(columnLabel);
		if (index == null) {
			throw new PSQLException("Column Index is null " + columnLabel, PSQLState.UNDEFINED_COLUMN);
		}
	...
	public Timestamp getTimestamp(int columnIndex) throws SQLException {
		try {
			long value = Long.parseLong(getValue(columnIndex));
	...
	@Override
	public boolean next() {
		final boolean hasNext = (cursor) < (rowset.size()-1);
		if (hasNext) {
			cursor++;
		}
