...
public class HeadersFilterTest extends AbstractWebIntegrationTest implements ApplicationListener<EmbeddedServletContainerInitializedEvent>{

	private static Logger LOGGER = LogManager.getLogger(HeadersFilterTest.class);
	...
	@Value("${custom.header.external-ip}")
	...
	public void setUp() throws Exception {
		...
		when(patientInListRepository.findByCriteria(patientSearchCriteria(), PatientSexEnum.FEMALE, AdmissionStatusEnum.UNKNOWN, "", null)).thenReturn(patientByCriteriaList());
	...
	public void testEndpointCalls(String authToken, ResultMatcher resultMatcher) throws Exception {
		...
		// Calling without the X-Forwarded-For must not return the FROM-INTERNET header
		mockMvc.perform(MockMvcRequestBuilders.get(baseUrl + patientDetailResourcePath, patient1Id)
				...
				.andExpect(MockMvcResultMatchers.header().doesNotExist(fromPublicNetworkHeader))
				.andExpect(MockMvcResultMatchers.header().string(CORSFilter.EXPOSE_HEADERS_HEADER, fromPublicNetworkHeader));
		mockMvc.perform(MockMvcRequestBuilders.get(baseUrl + companiesResourcePath)
				.header("Authorization", "Bearer " + allowedUserOAuth2AccessToken))
				.andExpect(MockMvcResultMatchers.header().doesNotExist(fromPublicNetworkHeader))
				.andExpect(MockMvcResultMatchers.header().string(CORSFilter.EXPOSE_HEADERS_HEADER, fromPublicNetworkHeader));

		// The value (normally IP address) in X-Forwarded must be the same as the value of custom header
		mockMvc.perform(MockMvcRequestBuilders.get(baseUrl + patientDetailResourcePath, patient1Id)
				.header("Authorization", "Bearer " + allowedUserOAuth2AccessToken)
				.header(RequestHelper.FORWARDED_FOR_HEADER, externalIpAddress))
	...
	@Test
	public void checkFromInternetHeaderWithOAuth2Token() throws Exception {
		String clientAuth = Base64Utils.encode(oAuth2ClientId + ":" + oAuth2ClientSecret);
		OAuth2AccessToken oAuth2Response = oAuth2Helper.accessToken(oAuth2TokenUrl, clientAuth, "password", allowedUsername, allowedPassword);
		allowedUserOAuth2AccessToken = oAuth2Response.getAccessToken();
		
		testEndpointCalls(allowedUserOAuth2AccessToken, MockMvcResultMatchers.header().string(fromPublicNetworkHeader, externalIpAddress));
	}

	@Test
	public void checkFromInternetHeaderWithTrustedPartnerJwt() throws Exception {
		...
		testEndpointCalls(jwt, MockMvcResultMatchers.header().string(fromPublicNetworkHeader, externalIpAddress));
	}

	@Test
	public void checkFromInternetHeaderWithSinglePatientJwt() throws Exception {
		ObjectMapper objectMapper = new ObjectMapper();
		JsonNode header = objectMapper.readTree(jwtHeader);
